...
class OdesiaTrainer(Seq2SeqTrainer):
    """
    The OdesiaTrainer is an adaptation of the 🤗 Transformers Trainer.
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if self.first_train_batch and self.tokenizer is not None:
            ...
            print_input_ids = inputs["input_ids"][:1].clone().detach().cpu()
            print_attention_mask = inputs["attention_mask"][:1].clone().detach().cpu()
            # print_labels = labels[:1].clone().detach().cpu()
            print_loss_weight_mask = loss_weight_mask[:1].clone().detach().cpu()
            ...
            # print("-- labels --")
            # if self.tokenizer is not None:
            #    print_labels[print_labels == -100] =first_train_batch self.tokenizer.pad_token_id
